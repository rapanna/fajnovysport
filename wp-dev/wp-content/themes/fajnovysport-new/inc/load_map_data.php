...
if($_GET["typ"] != "Sportoviste"){
    ...
    if($query->have_posts()){
        while ( $query->have_posts() ) {
                ...
                //featured image
                $thumbnail =  get_the_post_thumbnail_url(get_the_id(),"cahtegory-thumbnail");
                if(empty($thumbnail)){
                    $thumbnail = get_template_directory_uri().'/img/basic-image-icon.png';
                }

                $thumbnail_map =  get_the_post_thumbnail_url(get_the_id(),'map-left'); //90x90
                if(empty($thumbnail_map)){
                    $thumbnail_map = get_template_directory_uri().'/img/basic-image-icon.png';
                }
                $data->featuredImage = $featuredImage;
                $data->Data[$i] = (object)array();
    ...
    $url = "https://mapy.ostrava.cz/arcgisserver/rest/services/SMO_E/Sportoviste_E_PRO_gZRq52__0L1GDejn7z45__b14__PWRcaRh85_wE/MapServer/0/query?where=1=1&geometryType=esriGeometryEnvelope&spatialRel=esriSpatialRelIntersects&outFields=*&returnGeometry=false&returnTrueCurves=false&returnIdsOnly=false&returnCountOnly=false&returnZ=false&returnM=false&returnDistinctValues=false&returnExtentsOnly=false&f=json&token=".generateTokenFajn();
    ...
    /*    foreach($sporty_list as $sport){
        ...
        if ($sporty) {
            ...
            $compare_sporty_list = array_map('strtolower', $compare_sporty_list);

            //clean ';'
            for ($l=0; $l < count($compare_sporty_list); $l++) {
                $compare_sporty_list[$l] = str_replace(";", "", $compare_sporty_list[$l]);
            }
        ...
        $sporty_list = array_filter($sporty_list, fn($value) => !is_null($value) && $value !== '');
        ...
        if ($filter_ok){
            ...
            if((!empty($foto_1))){
                ...
                $data->Data[$i]->thubmnail = get_template_directory_uri()."/img/basic-image-icon.png";
            }
            if((!empty($foto_1))){
                $data->Data[$i]->thubmnailMap = $foto_1;
            } else {
                $data->Data[$i]->thubmnailMap = get_template_directory_uri()."/img/basic-image-icon.png";
            }
